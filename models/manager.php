<?php
	...
	class Manager extends Connect{
		...
		# Metódo de INSERÇÃO personalizado para a tabela de compras...
		public function insert_purchase($table, $data){

			//conexão com banco de dados sem PDO
			include_once $GLOBALS['project_path']."/models/connect02.php";
			...
			for($i = 0; $i < count($data[1]); $i++){
				if($data[1][$i]['cantidad'] == null || $data[1][$i]['cantidad'] == 0){}else{
					...
					$manager = new Manager;
					$ejecutar = $manager->select_common("tb_clientes",null,null," WHERE id_cliente = $id_cliente");
					...
					for($i2 = 0; $i2 < $data[1][$i]['cantidad']; $i2++){
						...
						//preparando query apartir dos campos($fields) e os parametros de valores nomeados($values)
						$fields = "cliente_id, cliente_name, producto_id, producto_compra, precio_compra, fecha_compra, user_id";
						$query = "INSERT INTO $table ($fields) VALUES ('$values')";
			...
			pg_close($conn);
			return $result;
		}//Fechando o método insert_purchase
